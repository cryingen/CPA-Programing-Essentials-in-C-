...
int daysBetween(Date since, Date till) {
    if ((since.year > till.year) || (since.year <= till.year && since.month > till.month) 
        || (since.year <= till.year && since.month <= till.month && since.day > till.day))
        return 1;
    else
    {
            ...
            else
            {
                ...
                if(since.year != till.year)
                    if(isLeap(since.year))
                        ...
                        dayss = 365 - dayOfYear(since);
                else
                    dayss = dayOfYear(since);
                int daysBetYears = 0;
